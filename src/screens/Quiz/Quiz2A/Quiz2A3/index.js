...
export default function App() {
    ...
    const respostas = ['Irrelevante para estrutura', 
    'Amplia perdas', 
    'Aumenta retornos', 
    'Limitada a cargos executivos'];
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Final2A');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Final2A') }
                ]
            );
        }
    };
    ...
    return (
        <StyledView>
            ...
            <PerguntaV
                pergunta="Impacto da alavancagem financeira?"
