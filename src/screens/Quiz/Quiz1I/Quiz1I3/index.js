...
export default function App() {
    ...
    const respostas = ['Lista de participantes e suas funções', 'Descrição detalhada de cada atividade', 'Sequência de atividades ao longo do tempo', 'Orçamento global do projeto'];
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Final1I');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Final1I') }
                ]
            );
        }
    };
