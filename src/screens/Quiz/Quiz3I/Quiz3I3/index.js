...
export default function App() {
    ...
    const respostas = ['Triagem e seleção', 
    'Apenas entrevistas', 
    'Ignorar candidatos', 
    'Apenas referências'];
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Final3I');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Final3I') }
                ]
            );
        }
    };
    ...
    return (
        <StyledView>
            ...
            <PerguntaV
                pergunta="Processo de recrutamento inclui?"
