...
export default function App() {
    ...
    const respostas = ['Observador passivo', 
        'Facilitador e apoiador', 
        'Sem impacto',
        'Apenas comunicador'];
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Quiz3A3');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Quiz3A3') }
                ]
            );
        }
    };
    ...
    return (
        <StyledView>
            ...
            <PerguntaV
                pergunta="Qual o papel do RH em mudanças organizacionais?"
