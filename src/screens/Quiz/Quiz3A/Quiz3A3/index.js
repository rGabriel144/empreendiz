...
export default function App() {
    ...
    const respostas = ['Desnecessário', 
    'Somente para executivos', 
    'Sim, em todos os níveis', 
    'Limitado a cargos intermediários'];
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Final3A');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Final3A') }
                ]
            );
        }
    };
    ...
    return (
        <StyledView>
            ...
            <PerguntaV
                pergunta="Desenvolvimento de liderança é vital?"
