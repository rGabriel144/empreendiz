...
export default function App() {
    ...
    const respostas = ['São iguais', 
        'Custos operacionais', 
        'Custo médio ponderado e retorno mínimo',
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Quiz2A3');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Quiz2A3') }
                ]
            );
        }
    };
    ...
    return (
        <StyledView>
            ...
            <PerguntaV
                pergunta="Diferença entre custo de capital e retorno exigido?"
