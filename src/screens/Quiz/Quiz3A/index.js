...
export default function App() {
    ...
    const respostas = ['Alta rotatividade', 
        'Cultura positiva', 
        'Ignorar motivação',
        'Reduzir salários'];
    ...
    const proximapergunta = async () => {
        if (respostax.isCorrect) {
            ...
            navigation.navigate('Quiz3A2');
        } else {
            Alert.alert(
                'Resposta Incorreta',
                `A resposta correta é: ${respostas[respostaCorreta]}`,
                [
                    { text: 'Próxima Pergunta', onPress: () => navigation.navigate('Quiz3A2') }
                ]
            );
        }
    };
    ...
    return (
        <StyledView>
            ...
            <PerguntaV
                pergunta="Estratégias de retenção incluem?"
